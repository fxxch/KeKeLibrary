...
+ (NetworkHelper *)defaultManager{
    static NetworkHelper *NetworkHelper_default = nil;
...
+ (NSString *)getNetworkType {
    if (![NetworkHelper whetherConnectedNetwork]) return @"NONE";
...
+ (NSString *)getSignalStrength {
    if (![NetworkHelper whetherConnectedNetwork]) return @"";
    UIApplication *app = [UIApplication sharedApplication];
    NSArray *subviews = [[[app valueForKey:@"statusBar"] valueForKey:@"foregroundView"] subviews];
    NSString *dataNetworkItemView = nil;
    NSString *signalStrength = @"";
    for (id subview in subviews) {
        if([subview isKindOfClass:[NSClassFromString(@"UIStatusBarDataNetworkItemView") class]] && [[NetworkHelper getNetworkType] isEqualToString:@"WIFI"] && ![[NetworkHelper getNetworkType] isEqualToString:@"NONE"]) {
            dataNetworkItemView = subview;
            signalStrength = [NSString stringWithFormat:@"%@dBm",[dataNetworkItemView valueForKey:@"_wifiStrengthRaw"]];
            break;
        }
        if ([subview isKindOfClass:[NSClassFromString(@"UIStatusBarSignalStrengthItemView") class]] && ![[NetworkHelper getNetworkType] isEqualToString:@"WIFI"] && ![[NetworkHelper getNetworkType] isEqualToString:@"NONE"]) {
            dataNetworkItemView = subview;
