...
static KKWindowModalView  *static_WindowModalView;
static UIWindow  *WindowModalView_currentKeyWindow;

@interface KKWindowModalView ()<UIScrollViewDelegate>
...
@implementation KKWindowModalView
...
+ (KKWindowModalView*)showWithItems:(NSArray<KKWindowModalViewItem*>*)aItemArray
    ...
    KKWindowModalView *modalView = [[KKWindowModalView alloc] initWithItems:aItemArray delegate:aDelegate];
...
#pragma mark == 初始化
#pragma mark ==================================================
- (instancetype)initWithItems:(NSArray<KKWindowModalViewItem*>*)aItemArray
                     delegate:(id<WindowModalViewDelegate>)aDelegate
{
    self = [super init];
    if (self) {
        ...
        self.pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.contentScrollView.frame)-KKSafeAreaBottomHeight-20, self.contentScrollView.frame.size.width, 20)];
        ...
        for (NSInteger i=0; i<[self.itemArray count]; i++) {
            ...
            KKWindowModalViewItem *item = [self.itemArray objectAtIndex:i];
            ...
            [button setImage:item.image
                       title:item.title
                  titleColor:item.titleColor?item.titleColor:[UIColor kk_colorWithHexString:@"#333333"]
             backgroundColor:nil
            ...
            if ((i%countPerRow)==(countPerRow-1)) {
                ...
                if (rowIndex==0) {
                    ...
                    if (i==[self.itemArray count]-1) {
                        [self.contentScrollView setContentSize:CGSizeMake(pageIndex*self.contentScrollView.frame.size.width, self.contentScrollView.frame.size.height)];
                    } else {
                        [self.contentScrollView setContentSize:CGSizeMake((pageIndex+1)*self.contentScrollView.frame.size.width, self.contentScrollView.frame.size.height)];
                    }
                }
            }
            else{
                offsetX = offsetX + frameWidth;
                [self.pageControl setNumberOfPages:pageIndex+1];
            }
        }
