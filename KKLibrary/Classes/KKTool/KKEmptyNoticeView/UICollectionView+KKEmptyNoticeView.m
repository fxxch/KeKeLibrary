//
//  UICollectionView+KKEmptyNoticeView.m
//  YouJia
//
...
@implementation UICollectionView (KKEmptyNoticeView)
...
- (void)setEmptyDataViewDelegate:(id<UICollectionViewEmptyNoticeViewDelegate>)emptyDataViewDelegate{
    objc_setAssociatedObject(self, @"EmptyNoticeViewDelegate",emptyDataViewDelegate, OBJC_ASSOCIATION_ASSIGN);
}

- (id<UICollectionViewEmptyNoticeViewDelegate>)emptyDataViewDelegate{
    id<UICollectionViewEmptyNoticeViewDelegate> delegate = objc_getAssociatedObject(self, @"EmptyNoticeViewDelegate");
    return delegate;
}
...
- (void)kk_ShowEmptyNoticeView{
    ...
    UIView *backgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, self.emptyDataOffsetY, fullView.frame.size.width, fullView.frame.size.height-self.emptyDataOffsetY)];
    ...
    if (self.emptyDataIconImage.size.width>fullView.frame.size.width) {
        CGFloat width = fullView.frame.size.width;
    ...
    CGSize textSize = [self.emptyDataText sizeWithFont:textFont
                                               maxSize:CGSizeMake(fullView.bounds.size.width-30, 10000)];
    CGFloat Y = 0;
    if (self.emptyDataTextAlignment==KKEmptyNoticeViewAlignment_Top) {
        ...
        if (self.emptyDataButton==nil) {
            ...
            Y = (fullView.bounds.size.height-imageSize.height-spaceBetween_imagetext-textSize.height)/2.0;
        }
        else{
            Y = (fullView.bounds.size.height-imageSize.height-spaceBetween_imagetext-textSize.height-spaceBetween_textbutton-button_height)/2.0;
        }
    }
    else{
    ...
    if (self.emptyDataIconImage) {
        ...
        [fullView addSubview:buton];
    }
    
    if ([NSString isStringNotEmpty:self.emptyDataText]) {
        ...
        [fullView addSubview:vTitleLabel];
        Y = Y + textSize.height;
    ...
    if (self.emptyDataButton) {
        self.emptyDataButton.frame = CGRectMake((fullView.bounds.size.width-self.emptyDataButton.frame.size.width)/2.0, Y, self.emptyDataButton.frame.size.width, button_height);
        [self.emptyDataButton addTarget:self action:@selector(emptyDataButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
        [fullView addSubview:self.emptyDataButton];
    }
}
...
- (void)emptyDataIconImageClicked{
    if (self.emptyDataViewDelegate && [self.emptyDataViewDelegate respondsToSelector:@selector(collectionEmptyNoticeView_emptyDataIconImageClicked)]) {
        [self.emptyDataViewDelegate collectionEmptyNoticeView_emptyDataIconImageClicked];
    }
}

- (void)emptyDataTextClicked{
    if (self.emptyDataViewDelegate && [self.emptyDataViewDelegate respondsToSelector:@selector(collectionEmptyNoticeView_emptyDataTextClicked)]) {
        [self.emptyDataViewDelegate collectionEmptyNoticeView_emptyDataTextClicked];
    }
}

- (void)emptyDataButtonClicked:(KKButton*)aButton{
    if (self.emptyDataViewDelegate && [self.emptyDataViewDelegate respondsToSelector:@selector(collectionEmptyNoticeView_emptyDataButtonClicked:)]) {
        [self.emptyDataViewDelegate collectionEmptyNoticeView_emptyDataButtonClicked:self.emptyDataButton];
    }
}
