...
@property (nonatomic,strong)NSMutableDictionary *isImageLoadingFromURLForState;
...
- (void)dealloc
{
    [self.textLabel removeObserver:self forKeyPath:@"font"];
}
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        self.imageViewSize = CGSizeMake(0, 0);
        self.edgeInsets = UIEdgeInsetsMake(0, 0, 0, 0);
        self.spaceBetweenImgTitle = 0;
        ...
        [self.textLabel addObserver:self forKeyPath:@"font" options:NSKeyValueObservingOptionNew context:nil];
    }
    return self;
}
...
- (instancetype)initWithFrame:(CGRect)frame type:(KKButtonType)aType{
    ...
    if (self) {
        ...
        if (aType>=KKButtonType_ImgFull_TitleLeftTop) {
            self.imageViewSize = frame.size;
        }
        else{
...
- (void)setButtonType:(KKButtonType)buttonType{
    if (buttonType==_buttonType) {
        ...
        if (_buttonType>=KKButtonType_ImgFull_TitleLeftTop) {
            self.imageViewSize = self.frame.size;
        }
        [self reloadUI];
    }
}
...
- (void)setImageWithURL:(NSURL *)aUrl
    ...
    [self.imageView kk_setImageWithURL:aUrl placeholderImage:imageNormal completed:^(NSData *imageData, NSError *error, BOOL isFromRequest) {
        ...
        [weakself.isImageLoadingFromURLForState removeObjectForKey:key];
...
//    }
    ...
    /* 图片在左，文字在右，左右分散对齐 */
        ...
        CGFloat txt_y = (self.frame.size.height-titleSize.height)/2.0+top;
        CGFloat txt_w = titleSize.width;
        CGFloat txt_h = titleSize.height;
        self.textLabel.frame = CGRectMake(txt_x, txt_y, txt_w, txt_h);
        self.textLabel.textAlignment = NSTextAlignmentRight;
