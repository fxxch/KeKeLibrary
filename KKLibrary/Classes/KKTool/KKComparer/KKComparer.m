    ...
    if (key && inputArrary && outDic && outDicKeys) {
        ...
        for (NSDictionary * dic in inputArrary) {
            ...
            if ([NSString isStringNotEmpty:name]) {
                NSString *firstString = [name substringToIndex:1];
                unichar code = [[KKComparer realString_ForDuoYinZi:firstString] characterAtIndex:0];//获取第一个字符的UniCode
...
+ (BOOL)isString:(NSString*)oldString containStringByFirstCharacterCompare:(NSString*)compareString{
    ...
    if (oldString) {
        for (int m=0;m<[oldString length];m++) {
            ...
            NSString *firstString = [oldString substringWithRange:range];
            unichar code = [[KKComparer realString_ForDuoYinZi:firstString] characterAtIndex:0];//获取第一个字符的UniCode
    ...
    if (newString) {
        for (int m=0;m<[newString length];m++) {
            ...
            NSString *firstString = [newString substringWithRange:range];
            unichar code = [[KKComparer realString_ForDuoYinZi:firstString] characterAtIndex:0];//获取第一个字符的UniCode
            char firstLetter = characterFirstLetter(code);//根据UniCode，获取字符
        ...
        for (int i=0; i<[inputArrary count]; i++) {
            ...
            if (newName) {
                for (int m=0;m<[newName length];m++) {
                    ...
                    NSString *firstString = [newName substringWithRange:range];
                    unichar code = [[KKComparer realString_ForDuoYinZi:firstString] characterAtIndex:0];//获取第一个字符的UniCode
                    char firstLetter = characterFirstLetter(code);//根据UniCode，获取字符
...
#pragma mark == 多音字处理
#pragma mark ==================================================
+ (NSString*)realString_ForDuoYinZi:(NSString*)aString{
    if ([aString isEqualToString:@"长"]) {
        return @"场";
    }
    else{
        return aString;
