...
//        }
        ...
        for (NSInteger i=0; i<[aItemArray count]; i++) {
            ...
            CGFloat titleWidth = [title sizeWithFont:[UIFont systemFontOfSize:14] maxWidth:CGFLOAT_MAX].width;
            ...
            [button setBackgroundColor:[UIColor colorWithHexString:@"#262626"] forState:UIControlStateNormal];
            [button setTitle:title forState:UIControlStateNormal];
            [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            
            [buttons addObject:button];
            
            maxWidth = maxWidth + button.width + 1;
            ...
            if (i==0) {
                if (i==[aItemArray count]-1) {
                    ...
                    [button setCornerRadius:10 type:KKCornerRadiusType_LeftTop|KKCornerRadiusType_LeftBottom];
                }
            }
            else{
                if (i==[aItemArray count]-1) {
                    [button setCornerRadius:10 type:KKCornerRadiusType_RightTop|KKCornerRadiusType_RightBottom];
                }
            }
        }
        
        
        CGRect windowFrame = [aView convertRect:aView.bounds toView:[UIWindow currentKeyWindow]];
        ...
        if (KKStatusBarAndNavBarHeight+KKWindowMenuItem_Height+KKWindowMenuItem_ArrowHeight> windowFrame.origin.y) {
            ...
            [self.boxView setCornerRadius:10];
            ...
            if (maxWidth>KKApplicationWidth-30) {
                ...
                [self.boxView setCenterX:windowFrame.origin.x+windowFrame.size.width/2.0];
                
                if (self.boxView.right>KKApplicationWidth) {
                    [self.boxView setRight:KKApplicationWidth-15];
                }
                else{
                    if (self.boxView.left<0) {
                        [self.boxView setLeft:15];
                    }
                }
            }
            ...
            for (NSInteger i=0; i<[buttons count]; i++) {
                UIButton *button = [buttons objectAtIndex_Safe:i];
                [button setLeft:offsetX];
                [self.contentScrollView addSubview:button];
                offsetX = offsetX + button.width;

                UIView *line = [[UIView alloc] initWithFrame:CGRectMake(offsetX, 0, 1.0, button.height)];
                line.backgroundColor = [UIColor whiteColor];
                [self.contentScrollView addSubview:line];
                
                offsetX = offsetX + 1;
            }
            self.contentScrollView.contentSize = CGSizeMake(offsetX, self.contentScrollView.height);
            ...
            self.arrowView.backgroundColor = [UIColor colorWithHexString:@"#262626"];
            [self addSubview:self.arrowView];
            [self.arrowView setMaskWithPath:[self KKWindowMenuItemArrow_upPath:self.arrowView.frame]];
            [self.arrowView setCenterX:windowFrameCenterX];
            
            if (self.boxView.right<self.arrowView.right) {
                [self.boxView setRight:self.arrowView.right+15];
            }
        ...
        //顶部显示
            ...
            //黑色三角
            self.arrowView = [[UIView alloc] initWithFrame:CGRectMake(0, self.boxView.bottom, KKWindowMenuItem_ArrowWidth, KKWindowMenuItem_ArrowHeight)];
            self.arrowView.backgroundColor = [UIColor colorWithHexString:@"#262626"];
            [self addSubview:self.arrowView];
            [self.arrowView setMaskWithPath:[self KKWindowMenuItemArrow_downPath:self.arrowView.frame]];
            [self.arrowView setCenterX:windowFrameCenterX];
