...
@interface KKAlbumImageShowItemView ()

@property (nonatomic , assign) CGRect myImageViewOriginFrame;

@end

@implementation KKAlbumImageShowItemView
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        [self observeNotification:@"NotificationName_KKAlbumImageShowItemViewResetZoomScale" selector:@selector(Notification_KKAlbumImageShowItemViewResetZoomScale:)];
...
-(void) singleTap:(UITapGestureRecognizer*) tap {
    if (self.delegate && [self.delegate respondsToSelector:@selector(KKAlbumImageShowItemViewSingleTap:)]) {
        [self.delegate KKAlbumImageShowItemViewSingleTap:self];
    }
}

- (void)Notification_KKAlbumImageShowItemViewResetZoomScale:(NSNotification*)notice{
...
- (void)videoPlayButtonClicked{
    ...
    if (image) {
        if (self.player) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(KKAlbumImageShowItemView:playVideo:)]) {
                [self.delegate KKAlbumImageShowItemView:self playVideo:YES];
            }
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(KKAlbumImageShowItemView:playVideo:)]) {
            [self.delegate KKAlbumImageShowItemView:self playVideo:NO];
        }
    }
}
        ...
        [[PHImageManager defaultManager] requestPlayerItemForVideo:self.assetModal.asset options:option resultHandler:^(AVPlayerItem *playerItem, NSDictionary *info) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (playerItem) {
                    ...
                    if (weakself.delegate && [weakself.delegate respondsToSelector:@selector(KKAlbumImageShowItemView:playVideo:)]) {
                        [weakself.delegate KKAlbumImageShowItemView:weakself playVideo:YES];
                    }
                }
            });
