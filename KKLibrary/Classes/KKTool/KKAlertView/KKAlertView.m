...
@property (nonatomic , copy) KKAlertViewButtonClickedBlock finishedBlock;
...
- (instancetype)initWithTitle:(NSString *)aTitle
                ...
                finishedBlock:(KKAlertViewButtonClickedBlock)aBlock
...
- (instancetype)initWithTitle:(NSString *)aTitle
    ...
    return [self initWithTitle:aTitle
                      ...
                      delegate:nil
                 finishedBlock:aBlock
             buttonTitlesArray:aButtonTitlesArray];
}
            ...
            buttonTitlesArray:(NSArray *)aButtonTitlesArray
{
    ...
    if (self) {
        ...
        [self kk_observeNotification:NotificationName_ShowKKAlertView selector:@selector(Notification_ShowAlertView:)];
        [self kk_observeNotification:NotificationName_HideKKAlertView selector:@selector(Notification_HideAlertView:)];
        ...
        [self.myBackgroundView kk_setCornerRadius:8.0];
        [self.myBackgroundView kk_setBorderColor:[UIColor colorWithRed:0.93 green:0.93 blue:0.93 alpha:1.0] width:0.5];
...
//    [window addSubview:self];
    ...
    [self kk_postNotification:NotificationName_HideKKAlertView object:self.myShowingIdentifier];
...
- (CGFloat)setAllText:(BOOL)aForReload{
    ...
    //只有标题
    if ([NSString kk_isStringNotEmpty:self.myTitleString] &&
        [NSString kk_isStringEmpty:self.mySubTitleString] &&
        [NSString kk_isStringEmpty:self.myMessageString]) {
        
        CGSize titleSize = [self.myTitleString kk_sizeWithFont:self.myTitleLabel.font maxSize:CGSizeMake(self.myBackgroundView.frame.size.width-20, 1000)];
    ...
    //只有副标题
    else if([NSString kk_isStringEmpty:self.myTitleString] &&
            [NSString kk_isStringNotEmpty:self.mySubTitleString] &&
            [NSString kk_isStringEmpty:self.myMessageString]){
        
        CGSize subTitleSize = [self.mySubTitleString kk_sizeWithFont:self.mySubTitleLabel.font maxSize:CGSizeMake(self.myBackgroundView.frame.size.width-20, 1000)];
    ...
    //只有文字
            ...
            [NSString kk_isStringEmpty:self.mySubTitleString] &&
            [NSString kk_isStringNotEmpty:self.myMessageString]){
        ...
        if (aForReload==NO) {
            CGFloat fontHeight2 = [UIFont kk_heightForFont:self.myMessageTextView.font numberOfLines:2];
            if (messageSize.height<=fontHeight2) {
                self.myMessageTextView.textAlignment = NSTextAlignmentCenter;
            }
            else{
    ...
    //标题+副标题
    else if([NSString kk_isStringNotEmpty:self.myTitleString] &&
            [NSString kk_isStringNotEmpty:self.mySubTitleString] &&
            [NSString kk_isStringEmpty:self.myMessageString]){
