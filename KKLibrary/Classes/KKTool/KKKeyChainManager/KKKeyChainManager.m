...
+ (BOOL)saveOject:(id)object forKey:(NSString *)key{
    ...
    if (deleteErr!=noErr &&
        deleteErr!= -25366 &&
        deleteErr!= -25300) {
        KKLogDebugFormat(@"keychainDeleteError: %ld",(long)deleteErr);
    }
    ...
    if (writeErr != errSecSuccess) {
        KKLogWarningFormat(@"Add KeyChain Item Error!!! Error Detail:%ld", (long)writeErr);
...
+ (id)readObjectForKey:(NSString *)key {
    ...
    if (SecItemCopyMatching((__bridge_retained CFDictionaryRef)keychainQuery, (CFTypeRef *)&keyData) == noErr) {
        @try {
            ...
            KKLogWarningFormat(@"Unarchive of %@ failed: %@", KKValidString(key), KKValidString(e));
        } @finally {
        }
    }
...
+ (BOOL)removeObjectForKey:(NSString *)key {
    ...
    if (deleteErr != errSecSuccess) {
        KKLogWarningFormat(@"delete UUID from KeyChain Error!!! Error code:%ld", (long)deleteErr);
...
+ (NSMutableDictionary *)getKeychainQueryForKey:(NSString *)akey{
    ...
    //密钥类型键 kSecClass
    [keychainQuery setObject:(__bridge_transfer NSString*)kSecClassGenericPassword
                      forKey:(__bridge_transfer NSString*)kSecClass];

    [keychainQuery setObject:akey
                      forKey:(__bridge_transfer NSString*)kSecAttrService];

    [keychainQuery setObject:akey
                      forKey:(__bridge_transfer NSString*)kSecAttrAccount];

    [keychainQuery setObject:(__bridge_transfer NSString*)kSecAttrAccessibleAlwaysThisDeviceOnly
                      forKey:(__bridge_transfer NSString*)kSecAttrAccessible];
    ...
    if (accessGroup != nil)
    {
        ...
        [keychainQuery setObject:accessGroup
                          forKey:(__bridge_transfer NSString*)kSecAttrAccessGroup];
#endif
    }
