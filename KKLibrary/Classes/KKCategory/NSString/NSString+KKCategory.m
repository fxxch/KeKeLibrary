...
+ (nullable NSString *)kk_getParamByName:(nullable NSString *)paramName URLString:(nullable NSString *)aURL{
    ...
    // 执行匹配的过程
    NSArray *matches = [regex matchesInString:aURL
                                      options:0
                                        range:NSMakeRange(0, [aURL length])];
    for (NSTextCheckingResult *match in matches) {
        NSString *tagValue = [aURL substringWithRange:[match rangeAtIndex:2]];  // 分组2所对应的串
        return tagValue;
    }
    return @"";
}
...
+ (nullable NSDictionary *)kk_getParamsWithUrlString:(nullable NSString*)aURL{
    ...
    //先截取问号
    NSArray* allElements = [aURL componentsSeparatedByString:@"?"];
