...
- (instancetype)initWithTitle:(NSString *)aTitle
    ...
    if (self) {
        ...
        self.cancelButton.titleLabel.font = ButtonTitleFont;
        [self.cancelButton setTitle:self.myCancelButtonTitle?self.myCancelButtonTitle:KILocalization(@"取消") forState:UIControlStateNormal];
...
- (void)setAllButton:(CGFloat)yOffset{
    ...
    for (NSInteger i=0; i<[self.buttonTitlesArray count]; i++) {
        ...
        UIButton *button01 = [[UIButton alloc] initWithFrame:CGRectMake(0, Y, self.myBackgroundView.frame.size.width, ButtonHeight)];
    ...
    self.myBackgroundView.frame = CGRectMake(15, KKScreenHeight-15-ButtonHeight-15-Y, self.frame.size.width-30, Y);
    [self.myBackgroundView setCornerRadius:5];
    
    self.cancelButton.frame = CGRectMake(15, KKScreenHeight-15-ButtonHeight, self.frame.size.width-30, ButtonHeight);
